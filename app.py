...
if not poi_df.empty:
    ...
    # --- NEW: DATA DISCOVERY WINDOW ---
    with st.sidebar.expander("📊 Available Data Counts"):
    ...
    # We use a key to prevent the widget from freezing
    selected_types = st.sidebar.multiselect(
        "Business Categories", 
        options=all_cats,
        default=[t for t in ['motel', 'massage', 'nightclub'] if t in all_cats],
        key="cat_selector_v3"
    )
...
with c1:
    ...
    st_folium(m, width=900, height=500, key="nova_map_v3")

    if 'target' in locals() and target in st.session_state['scan_results']:
        st.markdown("---")
        st.subheader(f"📄 Intelligence Report: {target}")
        st.info(f"**Flags:** {', '.join(st.session_state['scan_results'][target])}")

with c2:
    st.metric("Visible Dots", len(final_df))
